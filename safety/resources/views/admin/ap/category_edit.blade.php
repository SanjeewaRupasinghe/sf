...
@section('content')
<h1 class="h3 mb-4 text-gray-800">Edit Appraisal Category 
    <a href="{{route('apCategory.index')}}" class="btn btn-info float-right">Back</a></h1>
    
    <div class="card shadow mb-4">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-12" >
                    <form action="{{route('apCategory.update',$category->id)}}" method="post"  name="fname" enctype="multipart/form-data" role="form" id="formm">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-sm-8">
                                ...
                                <div class="tree-container">      
                                    <div class="tree-item active  {{ ($categories)?'has-children':''   }}">
                                    ...
                                    <div class="tree-item active  {{ ($cat->children)?'has-children':''   }}">
                                        ...
                                        @if(count($cat->children))
                                            @include('admin.ap.categoryChild_edit',['children' => $cat->children])
                                        @endif                                
                                        </div>
                                    @endforeach
                                    </div>
...
<script>
    $(function() {
        $('#name,#slug').blur(function () {
            $.ajax({
                method: "GET",
                url: "{{route('apCategory.checkSlug7')}}",
                data: {"name": $(this).val()}
            }).done(function (data) {
                $('#slug').val(data.slug);
            });
