 ...
 */
class MecShortCodeDesignerSpeaker extends Widget_Base {
	...
	public function get_title() {
		return __( 'MEC Speaker', 'mec-shortcode-designer' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .mec-speaker-name',
			]
		);
		$this->add_control(
			'address_align',
			[
				...
				'default'   => 'left',
				'selectors' => [
					'{{WRAPPER}} .mec-speaker-name' => 'text-align: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .mec-speaker-name' => 'display: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '#000',
				'selectors' => [
					'{{WRAPPER}} .mec-speaker-name' => 'color: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'default'   => '#40d9f1',
				'selectors' => [
					'{{WRAPPER}} .mec-speaker-name:hover' => 'color: {{VALUE}}',
				],
			]
		);
		$this->end_controls_section();
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .mec-speaker-name' => 'background: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'default'   => 'rgba(255,255,255,0)',
				'selectors' => [
					'{{WRAPPER}} .mec-speaker-name:hover' => 'background: {{VALUE}}',
				],
			]
		);
		$this->end_controls_section();
		...
		$this->add_control(
			...
			[
				...
				'selectors'  => [
					'{{WRAPPER}} .mec-speaker-name' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors'  => [
					'{{WRAPPER}} .mec-speaker-name' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_group_control(
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors' => [
					'{{WRAPPER}} .mec-speaker-name' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_group_control(
	...
	protected function render() {
		...
		if(!taxonomy_exists( 'mec_speaker' )){
		...
		$speakers = wp_get_post_terms( $event_id, 'mec_speaker' );
		$speaker_id   = get_post_meta( $event_id, 'mec_speaker_id', true );
		if(!empty($speakers) && is_array($speakers)){
			...
			foreach($speakers as $speaker){
				...
				$id = $speaker->term_taxonomy_id;
				$speaker_name = $speaker->name;

				$speakers_names[$id] = $speaker_name;
			}

			if( !empty($speakers_names) ){

				?>
				<div class="mec-shortcode-designer">
					<div class="mec-speaker-name"><?php echo esc_html( implode(__(' , ','mec-shortcode-designer'),$speakers_names) ); ?></div>
				</div>
				<?php
			}
			...
			?>
			<div class="mec-shortcode-designer">
				<div class="mec-speaker-name"><?php echo esc_html( 'Speaker Name' ); ?></div>
			</div>
			<?php
		}
	}
