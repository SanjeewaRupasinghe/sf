 ...
 */
class MecShortCodeDesignerTime extends Widget_Base {
	...
	public function get_title() {
		return __( 'MEC Time', 'mec-shortcode-designer' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_group_control(
			...
			[
				...
				'selector'	=> '{{WRAPPER}} .mec-shortcode-designer .mec-event-time, {{WRAPPER}} .mec-shortcode-designer .mec-event-time .mec-time-details',
			]
		);
		$this->add_control(
			'title_align',
			[
				...
				'default' => 'left',
				'selectors' => [
					'{{WRAPPER}} .mec-event-time' => 'text-align: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .mec-event-time' => 'display: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'	=> '#000',
				'selectors' => [
					'{{WRAPPER}} .mec-event-time' => 'color: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'default'	=> '#000',
				'selectors' => [
					'{{WRAPPER}} .mec-event-time:hover' => 'color: {{VALUE}}',
				],
			]
		);
		$this->end_controls_section();
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .mec-event-time' => 'background: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'default'	=> 'rgba(255,255,255,0)',
				'selectors' => [
					'{{WRAPPER}} .mec-event-time:hover' => 'background: {{VALUE}}',
				],
			]
		);
		$this->end_controls_section();
		...
		$this->add_control(
			...
			[
				...
				'selectors'		=> [
					'{{WRAPPER}} .mec-event-time' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units'	=> [ 'px', '%', 'em' ],
				'selectors'		=> [
					'{{WRAPPER}} .mec-event-time' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_group_control(
			...
			[
				...
				'selector'	=> '{{WRAPPER}} .mec-event-time',
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors' => [
					'{{WRAPPER}} .mec-event-time' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_group_control(
	...
	protected function render() {
		...
		$datetimes = EventsDateTimes::instance()->get_datetimes($event_id,'time'.$MEC_Shortcode_id);
		...
		if(empty($datetimes)){
			...
			$start_event_time =  "$s_h:$s_m:00 $s_am_pm";
			$end_event_time = "$e_h:$e_m:00 $e_am_pm";
			$start_event_time = date($time_format,strtotime($start_event_time));
			$end_event_time = date($time_format,strtotime($end_event_time));
		}
		$is_all_day = get_post_meta( $event_id, 'mec_allday', true) == '1' ;

		$start_time =  isset($datetimes['start']['time']) && !empty($datetimes['start']['time']) ? $datetimes['start']['time'] : $start_event_time;
		$end_time = isset($datetimes['end']['time']) && !empty($datetimes['end']['time']) ? $datetimes['end']['time'] : $end_event_time;
		...
		<div class="mec-shortcode-designer">
			<div class="mec-event-time">
				...
				if( $is_all_day ){
					...
					echo $main->display_time($start_time,$end_time);
				}
				?>
			</div>
		</div>
