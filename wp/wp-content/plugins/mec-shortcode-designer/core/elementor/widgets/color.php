 ...
 */
class MecShortCodeDesignerColor extends Widget_Base {
	...
	public function get_title() {
		return __( 'MEC Color', 'mec-shortcode-designer' );
	}
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			'content_section',
			[
				'label' => __( 'Content', 'mec-shortcode-designer' ),
		...
		$this->add_control(
			'event_color',
			[
				'label'			=> __( 'Event Color as background', 'mec-shortcode-designer' ),
				'type'			=> Controls_Manager::SWITCHER,
				'label_on'		=> __( 'Yes', 'mec-shortcode-designer' ),
				'label_off'		=> __( 'No', 'mec-shortcode-designer' ),
				'return_value'	=> 'yes',
				'default'		=> 'no',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => 'left',
				'selectors' => [
					'{{WRAPPER}} .mec-event-color' => 'text-align: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .mec-event-color' => 'display: {{VALUE}}',
				],
			]
		);
		$this->end_controls_section();
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .mec-event-color' => 'background: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'default'	=> 'rgba(255,255,255,0)',
				'selectors' => [
					'{{WRAPPER}} .mec-event-color:hover' => 'background: {{VALUE}}',
				],
			]
		);
		$this->end_controls_section();
		...
		$this->add_control(
			...
			[
				...
				'selectors'		=> [
					'{{WRAPPER}} .mec-event-color' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units'	=> [ 'px', '%', 'em' ],
				'selectors'		=> [
					'{{WRAPPER}} .mec-event-color' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_group_control(
			...
			[
				...
				'selector'	=> '{{WRAPPER}} .mec-event-color',
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors' => [
					'{{WRAPPER}} .mec-event-color' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_group_control(
		...
		if ( get_post_type() == 'mec_designer' ) {
			...
			<div class="mec-shortcode-designer">
				<div class="mec-event-color">
					<span class="event-color" style="background:#<?php echo get_post_meta( $event_id, 'mec_color', true ); ?>"></span>
				</div>
			</div>
			<?php
		} else {
			if ( $settings['event_color'] == 'yes' ) {
				echo '<script>jQuery(".mec-sd-event-article'. get_the_ID().'").css({background: "#'. get_post_meta( get_the_ID(), 'mec_color', true ).'"})</script>';
			} else {
			?>
			<div class="mec-shortcode-designer">
				<div class="mec-event-color">
					<span class="event-color" style="background:#<?php echo get_post_meta( get_the_ID(), 'mec_color', true ); ?>"></span>
				</div>
			</div>
			<?php
			}
