...
$q="select Product_Name,Description,Product_Code,Product_Category,Qty_in_Stock,Unit_Price,Tag,Created_Time,Modified_Time from Products  where Modified_Time > '".$d->format('c')."'  order by Modified_Time asc";//; // $this->items_batch
if(!empty($meta['page'])){
...
$type=$info['data']['type'];
//$this->items_batch='300'; $meta['page']=3;
...
//$res=$api->post_crm('items?search_text=WEYESVPN18L0.75'); //2019-11-27T12:05:02-0500
//echo json_encode($res); die(); //for incventory actual_available_stock and available_stock
...
if(!empty($res['items'])){
 ...
 foreach($res['items'] as $v){
   ...
   if(isset($v['actual_available_stock']) && !isset($v['stock_on_hand'])){
   ...
   if(isset($v['stock_on_hand'])){
     ...
     foreach($item['item']['warehouses'] as $w){
        ...
        $v['stock_on_hand']=floatval($w['warehouse_actual_available_for_sale_stock']);  //warehouse_stock_on_hand , warehouse_available_stock ,warehouse_available_for_sale_stock   
        } 
     }    
     }     
       }
...
/************** save product ********************************/
//sync stock
$log.=$this->sync_item($p,$item,$meta,$pro_id);
...
$orders=array();
if(empty($info['data']['type'])){
   $q="select Subject,Status,Created_Time,Modified_Time from Sales_Orders where Modified_Time > '".$d_order->format('c')."'  order by Modified_Time desc  limit ".$this->items_batch; 
...
$type=$info['data']['type'];
$res=$api->post_crm('salesorders?sort_column=last_modified_time&per_page=30&last_modified_time='.urlencode($d_order->format('Y-m-d\TH:i:sO')));
...
if(!empty($posts)){
     foreach($posts as $v){
      ...
      $orders_found[$order_id]=$v['meta_value'];
        if($order && isset($order_status_arr[$v['meta_value']])){
 $zoho_status=$order_status_arr[$v['meta_value']];
            $new_status=$this->get_woo_status($meta,$zoho_status);
           //var_dump($new_status,$woo_status,$zoho_status,$order_id); die('----------');
          if(!empty($new_status) && $new_status != $woo_status ){
          $orders_done[$v['meta_value']]='Status of woo #'.$order_id.' and zoho #'.$v['meta_value'].' is '.$new_status;      
...
if(!empty($after)){
    ...
    //increase 1 second for books , inventory etc
    if(!empty($meta['after']) && $meta['after'] == $after){
        if(empty($meta['page'])){ $meta['page']=1; }
     $meta['page']= $meta['page']+1;  
    }else{
     $meta['page']=0;    
    }
...
if(in_array($p_type,array('simple','variation')) && !empty($meta['sync_items'])){      
...
$sections['cron']=__('Zoho CRM to WooCommerce','woocommerce-zoho-crm');
return $sections;
}
...
<tr>
<th><label for="vx_ac"><?php _e('Zoho Account','woocommerce-zoho-crm'); ?></label></th>
<td>  <select id="vxc_account" name="crm[account]" style="width: 100%">
<option value=""><?php _e('Select Account','woocommerce-zoho-crm'); ?></option>
<?php
...
$(document).on("click",".vx_refresh_data",function(e){
 ...
 if(re.status == "ok"){
  ...
  if(re.error && re.error!=""){
   ...
   add_sel2();  
function add_sel2(){
//jQuery('.vx_item_fields').select2({ placeholder: '<?php _e('Select Item Field','woocommerce-zoho-crm') ?>'});
//jQuery('#crm_sel_attr').select2({ placeholder: '<?php _e('Select Attributes','woocommerce-zoho-crm') ?>'});
//jQuery('#crm_sel_book').select2({ placeholder: '<?php _e('Select Price Book','woocommerce-zoho-crm') ?>'});
jQuery('#crm_sel_cats').select2({ placeholder: '<?php _e('Select categories','woocommerce-zoho-crm') ?>'});
...
// if(!empty($cron['next'])){
  ?>
  <div class="updated below-h2"><h4><?php _e('Products Sync CRON','woocommerce-zoho-crm'); ?></h4>
  <?php
      if(!empty($cron['last'])){
  ?>
...
$zoho_status=array('inventory'=>array('fulfilled','confirmed','void','overdue'),'books'=>array('draft','open', 'invoiced','partially_invoiced','void','overdue'),'invoice'=>array('sent', 'draft','overdue','paid','void','unpaid','partially_paid','viewed'),''=>array('Created','Approved','Delivered','Cancelled'));
//unset($meta['zoho']);
$zoho_statuses=array();
...
<table class="form-table">
<tr>
<th><label for="vx_product"><?php _e('Sync Products','woocommerce-zoho-crm');  ?></label></th>
<td><label for="vx_product"><input type="checkbox" id="vx_product" name="crm[sync_items]" value="yes" <?php if(!empty($meta['sync_items'])){echo 'checked="checked"';} ?> > <?php _e('Add Zoho Products to WooCommerce','woocommerce-zoho-crm');  ?></label></td>
</tr>

<tr>
<th><label for="vx_inventory"><?php _e('Sync Inventory','woocommerce-zoho-crm');  ?></label></th>
<td><label for="vx_inventory"><input type="checkbox" id="vx_inventory" name="crm[sync_stock]" value="yes" <?php if(!empty($meta['sync_stock'])){echo 'checked="checked"';} ?> > <?php _e('Update Qty on Hand from Zoho to WooCommerce','woocommerce-zoho-crm');  ?></label></td>
</tr>

<tr>
<th><label for="vx_title"><?php _e('Sync Product Title','woocommerce-zoho-crm');  ?></label></th>
<td><label for="vx_title"><input type="checkbox" id="vx_title" name="crm[sync_title]" value="yes" <?php if(!empty($meta['sync_title'])){echo 'checked="checked"';} ?> > <?php _e('Sync Product title and Short Description from Zoho to WooCommerce if product already exists in WooCommerce.','woocommerce-zoho-crm');  ?></label></td>
</tr>

<tr>
<th><label for="vx_item_cron"><?php _e('Sync Time','woocommerce-zoho-crm'); ?></label></th>
...
<tr>
<th><label><?php _e('Sync Products Updated after','woocommerce-zoho-crm'); ?></label></th>
<td>
<input type="text" name="crm[after]" class="vxc_date" value="<?php echo $after; ?>" autocomplete="off" style="width: 99%">
 <div class="howto"><?php _e('Leave empty to start from 0 ','woocommerce-zoho-crm') ?></div> 
...
<tr>
<th><label><?php _e('Allow BackOrders','woocommerce-zoho-crm'); ?></label></th>
<td>
...
<select  name="crm[back_order]" style="width: 99%">
 ...
 <tr>
<th><label><?php _e('Sync if category is','woocommerce-zoho-crm'); ?></label></th>
<td>
  <div class="vx_tr" >
  <div class="vx_td">
<select class="vx_sel2" id="crm_sel_cats" name="crm[item_cats][]" multiple="multiple" style="width: 99%">
 ...
 <div class="howto"><?php _e('Leave empty to sync items from all categories','woocommerce-zoho-crm') ?></div> 
  </div>
  <div class="vx_td2">
  <button class="button vx_refresh_data" data-id="refresh_cats">
  <span class="reg_ok"><i class="fa fa-refresh"></i> <?php _e('Refresh','woocommerce-zoho-crm') ?></span>
  <span class="reg_proc" style="display: none;"><i class="fa fa-circle-o-notch fa-spin"></i> <?php _e('Loading ...','woocommerce-zoho-crm') ?></span>
  </button>
  ...
  <tr>
<th><label for="vx_stock"><?php _e('If out of stock in Zoho','woocommerce-zoho-crm');  ?></label></th>
...
?>
  <tr>
<th><label for="vx_warehouse"><?php _e('Select Warehouse for Stock Qty','woocommerce-zoho-crm');  ?></label></th>
...
<h3><?php _e('Sync Zoho Orders Status (Optional)','woocommerce-zoho-crm');  ?></h3>
<table class="form-table">
<tr>
<th><label for="vx_orders"><?php _e('Sync Order Status','woocommerce-zoho-crm');  ?></label></th>
<td><label for="vx_orders"><input type="checkbox" id="vx_orders" class="vx_toggles" name="crm[sync_order]" value="yes" <?php if(!empty($meta['sync_order'])){echo 'checked="checked"';} ?> > <?php _e('Synchronize Order Status from Zoho to WooCommerce.','woocommerce-zoho-crm');  ?></label></td>
</tr>
</table>
<div id="vx_orders_div" style="<?php if(empty($meta['sync_order'])){echo 'display:none';} ?>">
<table class="form-table">
<tr>
<th><label><?php _e('Sync Orders Updated after','woocommerce-zoho-crm'); ?></label></th>
<td>
...
<h3><?php _e('Map Zoho Order Status to WooCommerce Order Status','woocommerce-zoho-crm');  ?></h3>
<table class="form-table">
<tr>
<th>
<label><?php _e('Zoho Order Status','woocommerce-zoho-crm'); ?></label>
<div class="howto">e.g: <?php echo implode(', ',$zoho_statuses); ?></div>
</th>
<th><label><?php _e('WooCommerce Order Status','woocommerce-zoho-crm'); ?></label></th>
</tr>
<?php
...
<p class="submit_vx">
  <button type="submit" value="save" class="button-primary" title="<?php _e('Save Changes','woocommerce-zoho-crm'); ?>" name="save"><?php _e('Save Changes','woocommerce-zoho-crm'); ?></button>
</p>    
    <?php
