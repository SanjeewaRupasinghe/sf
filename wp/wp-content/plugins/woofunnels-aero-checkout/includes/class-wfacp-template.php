...
abstract class WFACP_Template_Common {
	...
	public function enqueue_script() {
		...
		if ( isset( $global_settings['wfacp_google_address_key'] ) && '' !== $global_settings['wfacp_google_address_key'] ) {
			...
			if ( isset( $page_settings['enable_google_autocomplete'] ) && wc_string_to_bool( $page_settings['enable_google_autocomplete'] ) ) {
				...
				wp_enqueue_script( 'wfacp_google_js', 'https://maps.googleapis.com/maps/api/js?key=' . $gmap_api_key . '&libraries=places&callback=funnelkit_checkout_map_init', [ 'wfacp_checkout_js' ], '', true );
			}
		}
	...
	public function process_phone_field() {
		...
		if ( wc_string_to_bool( $page_settings['enable_phone_validation'] ) ) {
			if ( isset( $_POST['billing_phone'] ) && ! empty( $_POST['billing_phone'] ) && isset( $phone_data['billing']['number'] ) && empty( $phone_data['billing']['number'] ) ) {
			...
			if ( isset( $_POST['shipping_phone'] ) && ! empty( $_POST['shipping_phone'] ) && isset( $phone_data['shipping']['number'] ) && empty( $phone_data['shipping']['number'] ) ) {
