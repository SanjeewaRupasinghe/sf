    ...
    public function output()
    {
        ...
        if($this->status)
        {
            ...
            $this->factory->params('footer', function() use($HTML)
            {
                ...
                {
                    $('hr.wp-header-end').before('<a href="<?php echo esc_url($this->main->remove_qs_var('admincalendar')); ?>" class="add-new-h2"><?php esc_html_e('Classic View', 'mec'); ?></a>');
        ...
        else
        {
            $this->factory->params('footer', function()
            {
                ...
                jQuery(document).ready(function($)
                {
                    $('hr.wp-header-end').before('<a href="<?php echo esc_url($this->main->add_qs_vars(array('admincalendar'=>1))); ?>" class="add-new-h2"><?php esc_html_e('Calendar View', 'mec'); ?></a>');
                });
                </script>
                <?php
            });
