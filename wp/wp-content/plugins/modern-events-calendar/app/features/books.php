...
class MEC_feature_books extends MEC_base
{
    ...
    ?>
        <div class="mec-book-details">
            ...
            <div class="mec-row">
                ...
                <span><?php echo esc_html($this->main->render_price(($meta['mec_price'] ? $meta['mec_price'] : 0), $event_id)); ?></span>
            </div>
            <div class="mec-row">
            ...
            <div class="mec-billing">
                <?php
                    if(isset($transaction['price_details']) and isset($transaction['price_details']['details']))
                    {
                        ...
                        {
                            echo '<div><strong>'.esc_html($price_row['description']).":</strong> ".esc_html($this->main->render_price($price_row['amount'], $event_id)).'</div>';
                        }

                        if(trim($coupon_code)) echo '<div><strong>'.esc_html__('Coupon Code', 'mec').':</strong> <code>'.esc_html($coupon_code).'</code></div>';
                        echo '<div><strong>'.esc_html__('Total', 'mec').':</strong> '.esc_html($this->main->render_price($transaction['price'], $event_id)).'</div>';
                    }
                ?>
            </div>
        </div>
                ...
                <div id="mec_book_edit_form_event_options">
                    ...
                    <div id="mec_date_tickets_booking_form_bfixed_fields">
                        ...
                        <div id="mec_date_tickets_booking_form_bfixed_list">
                            ...
                            <div class="mec-form-row mec-book-bfixed-field-<?php echo sanitize_html_class($bfixed_field['type']); ?> <?php echo ((isset($bfixed_field['mandatory']) and $bfixed_field['mandatory']) ? 'mec-reg-mandatory' : ''); ?>" data-field-id="<?php echo esc_attr($bfixed_field_id); ?>">
                                <div class="mec-col-2">
                                ...
                                <div class="mec-col-6">
                                    ...
                                    <label for="mec_book_bfixed_field_reg<?php echo esc_attr($bfixed_field_id.'_'.strtolower(str_replace(' ', '_', $bfixed_field_option['label']))); ?>">
                                        <input type="radio" id="mec_book_bfixed_field_reg<?php echo esc_attr($bfixed_field_id.'_'.strtolower(str_replace(' ', '_', $bfixed_field_option['label']))); ?>" name="mec_fields[<?php echo esc_attr($bfixed_field_id); ?>]" value="<?php esc_html_e($bfixed_field_option['label'], 'mec'); ?>" <?php echo (($bfixed_value == $bfixed_field_option['label']) ? 'checked="checked"' : ''); ?> />
                            ...
                            <?php $i = 0; foreach($attendees as $key => $attendee): $i = max($i, $key); $attachments = (isset($attendees['attachments']) and is_array($attendees['attachments'])) ? $attendees['attachments'] : NULL; ?>
                                ...
                                <div class="mec-attendee" id="mec_attendee<?php echo esc_attr($key); ?>">
                                    ...
                                    <div class="mec-form-row">
                                        ...
                                        <div class="mec-col-6">
                                            <select id="att_<?php echo esc_attr($key); ?>_ticket" name="mec_att[<?php echo esc_attr($key); ?>][id]" class="widefat mec-booking-edit-form-tickets">
                                                ...
                                                <div class="mec-col-6">
                                                    ...
                                                    <?php foreach($reg_field['options'] as $reg_field_option): ?>
                                                        <label for="mec_book_reg_field_reg<?php echo esc_attr($key.'_'.$reg_field_id.'_'.strtolower(str_replace(' ', '_', $reg_field_option['label']))); ?>">
                                                            <input type="radio" id="mec_book_reg_field_reg<?php echo esc_attr($key.'_'.$reg_field_id.'_'.strtolower(str_replace(' ', '_', $reg_field_option['label']))); ?>" name="mec_att[<?php echo esc_attr($key); ?>][reg][<?php echo esc_attr($reg_field_id); ?>]" value="<?php esc_html_e($reg_field_option['label'], 'mec'); ?>" <?php echo ((isset($attendee['reg']) and isset($attendee['reg'][$reg_field_id]) and $attendee['reg'][$reg_field_id] == $reg_field_option['label']) ? 'checked="checked"' : ''); ?> />
                                    ...
                                    <div class="mec-book-ticket-variations" data-key="<?php echo esc_attr($key); ?>">
    ...
    public function filter_columns_content($column_name, $post_id)
    {
        if($column_name == 'event')
        {
            ...
            echo ($event_id ? '<a href="'.esc_url($this->main->add_qs_var('mec_event_id', $event_id)).'">'.$title.'</a>' : '');
        ...
        elseif($column_name == 'price')
        {
            ...
            echo esc_html($this->main->render_price(($price ? $price : 0), $event_id));
    ...
    public function add_filters($post_type)
    {
        ...
        if($mec_event_id) echo MEC_kses::element($this->add_occurrence_filter($mec_event_id));
    ...
    public function do_bulk_actions()
    {
        ...
        switch($action)
        {
            case 'confirm':
                ...
                $post_ids = (isset($_REQUEST['post']) and is_array($_REQUEST['post']) and count($_REQUEST['post'])) ? array_map('sanitize_text_field', wp_unslash($_REQUEST['post'])) : array();
    ...
    {
        if(!$post_ids) $post_ids = (isset($_REQUEST['post']) and is_array($_REQUEST['post']) and count($_REQUEST['post'])) ? array_map('sanitize_text_field', wp_unslash($_REQUEST['post'])) : array();
    ...
    public function save_book($post_id)
    {
        ...
        if($is_new_booking)
        {
            ...
            // Register Attendee
            $attendees = (isset($_POST['mec_attendee']) and is_array($_POST['mec_attendee'])) ? $this->main->sanitize_deep_array($_POST['mec_attendee']) : array();

            $main_attendee = (isset($attendees[0]) and is_array($attendees[0])) ? $attendees[0] : array();
            ...
            {
                $attendee = (isset($attendees[$i]) and is_array($attendees[$i])) ? $attendees[$i] : array();
        ...
        if($is_edit_booking)
        {
            ...
            $mec_atts = (isset($_POST['mec_att']) and is_array($_POST['mec_att'])) ? $this->main->sanitize_deep_array($_POST['mec_att']) : array();

            // Booking Fixed Fields
            $bfixed_fields = (isset($_POST['mec_fields']) and is_array($_POST['mec_fields'])) ? $this->main->sanitize_deep_array($_POST['mec_fields']) : array();
    ...
    public function book()
    {
                ...
                if($files['name'][$key])
                {
                    foreach($files['name'][$key][1]['reg'] as $id => $reg)
                    {
                        if(!empty($files['name'][$key][1]['reg'][$id]))
                        {
                            ...
                            if(!$has_valid_type)
                            {
                                $this->main->response(array('success'=>0, 'message'=> '"'.$files['name'][$key][1]['reg'][$id] .'"<br />'.esc_html__('Uploaded file type is not supported.', 'mec')));
                                die();
                            }
        ...
        $book = $_REQUEST['book']; // Already Sanitized
        ...
        switch($step)
        {
            case '1':
                ...
                {
                    $g_recaptcha_response = isset($_REQUEST['g-recaptcha-response']) ? sanitize_text_field($_REQUEST['g-recaptcha-response']) : NULL;
    ...
    {
        $event_id = isset($_REQUEST['event_id']) ? sanitize_text_field($_REQUEST['event_id']) : '';
        $dates = (isset($_REQUEST['date']) and is_array($_REQUEST['date']) and count($_REQUEST['date'])) ? array_map('sanitize_text_field', wp_unslash($_REQUEST['date'])) : array();
    ...
    public function bbf_date_tickets_booking_form()
    {
        ...
        $ongoing_event_book = (isset($this->settings['booking_ongoing']) and $this->settings['booking_ongoing'] == '1') ? true : false;
        ...
        $output .= '<div class="mec-col-6">'.MEC_kses::form($date_options).'</div></div>';
        ...
        for($attendee_id = 0; $attendee_id < $num_attendees; $attendee_id++)
        {
            ...
            $booking_form_options .= '<div class="mec-col-6"><select class="widefat" name="mec_attendee['.esc_attr($attendee_id).'][id]" id="mec_attendee_'.esc_attr($attendee_id).'_ticket">'.$ticket_options.'</select></div></div>';
    ...
    public function bbf_event_edit_options()
    {
        ...
        $this->main->response(array('success' => 1, 'dates' => MEC_kses::form($date_options), 'tickets' => MEC_kses::form($ticket_options), 'variations' => MEC_kses::form($variation_options), 'reg_fields' => MEC_kses::form($booking_form_options)));
    }
    ...
    public function shortcode($atts)
    {
        ...
        return '<div class="mec-events-meta-group mec-events-meta-group-booking mec-events-meta-group-booking-shortcode">' . MEC_kses::full($this->main->module('booking.default', array(
            'event' => $events,
            'ticket_id' => $ticket_id,
            'from_shortcode' => true
        ))) . '</div>';
