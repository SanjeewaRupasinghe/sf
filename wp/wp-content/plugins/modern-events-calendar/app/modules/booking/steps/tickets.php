...
<form id="mec_book_form<?php echo esc_attr($uniqueid); ?>" onsubmit="mec_book_form_submit(event, <?php echo esc_attr($uniqueid); ?>);">
    ...
    <div class="mec-book-first">
        ...
        <?php else: ?>
        <label for="mec_book_form_date<?php echo esc_attr($uniqueid); ?>"><?php esc_html_e('Date', 'mec'); ?>: </label>
        ...
        <div class="mec-event-ticket mec-event-ticket<?php echo sanitize_html_class($ticket_limit); ?>" id="mec_event_ticket<?php echo esc_attr($ticket_id); ?>">
            <div class="mec-ticket-available-spots <?php echo ($ticket_limit == '0' ? 'mec-util-hidden' : ''); ?>">
                ...
                <p>
                    ...
                    <div class="mec-event-ticket-available"><?php echo sprintf(esc_html__('Available %s: <span>%s</span>', 'mec'), $this->m('tickets', esc_html__('Tickets', 'mec')), (($ticket['unlimited'] and $ticket_limit == '-1') ? esc_html__('Unlimited', 'mec') : ($ticket_limit != '-1' ? $ticket_limit : esc_html__('Unlimited', 'mec')))); ?></div>
                </p>
                <?php else: ?>
                <div>
                    <input onkeydown="return event.keyCode !== 69" type="number" class="mec-book-ticket-limit" name="book[tickets][<?php echo esc_attr($ticket_id); ?>]" title="<?php esc_attr_e('Count', 'mec'); ?>" placeholder="<?php esc_attr_e('Count', 'mec'); ?>" value="<?php echo esc_attr($default_ticket_number); ?>" min="0" max="<?php echo ($ticket_limit != '-1' ? $ticket_limit : ''); ?>" onchange="mec_check_tickets_availability<?php echo esc_attr($uniqueid); ?>(<?php echo esc_attr($ticket_id); ?>, this.value);" />
                </div>
                <span class="mec-event-ticket-available"><?php echo sprintf(esc_html__('Available %s: %s', 'mec'), $this->m('tickets', esc_html__('Tickets', 'mec')), (($ticket['unlimited'] and $ticket_limit == '-1') ? '<span>'.esc_html__('Unlimited', 'mec').'</span>' : ($ticket_limit != '-1' ? '<span>'.$ticket_limit.'</span>' : '<span>'.esc_html__('Unlimited', 'mec').'</span>'))); ?></span>
                <?php endif; ?>
            </div>
            <?php
    ...
    <button id="mec-book-form-btn-step-1" style="<?php if($available_spots == '0') echo 'display: none;'; ?>" type="submit" onclick="mec_book_form_back_btn_cache(this, <?php echo esc_attr($uniqueid); ?>);"><?php echo (($WC_status and !$WC_booking_form) ? esc_html__('Add to Cart', 'mec') : esc_html__('Next', 'mec')); ?></button>
</form>
