...
class MEC_main extends MEC_base
{
    ...
    public function get_sidebar_menu($active_menu = 'settings')
    {
        ...
        $settings = apply_filters('mec-settings-items-settings', array(
            ...
            esc_html__('Google Recaptcha', 'mec') => 'recaptcha_option',
            ...
            esc_html__('Search Bar', 'mec') => 'search_bar_options',
    ...
    public function save_options()
    {
        ...
        $wpnonce = sanitize_text_field($request->getVar('_wpnonce', NULL));
        ...
        {
            // Locale
            $locale = sanitize_text_field($request->getVar('mec_locale', NULL));
            if($locale)
            {
		...
		<div class="mec-wrap">
			<div class="mec-map-lightbox-wp mec-event-list-classic">
				<article class="'.((isset($event->data->meta['event_past']) and trim($event->data->meta['event_past'])) ? 'mec-past-event ' : '').'mec-event-article mec-clear">
                    ...
                    <div class="mec-map-time" style="display: none">'.MEC_kses::element($this->display_time($start_time, $end_time)).'</div>
    ...
    public function sn_twitter($url, $event)
    {
        ...
        return '<li class="mec-event-social-icon"><a class="twitter" href="https://twitter.com/share?url='.rawurlencode($url).'" onclick="javascript:window.open(this.href, \'\', \'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=500\'); return false;" target="_blank" title="'.esc_html__('Tweet', 'mec').'"><i class="mec-fa-twitter"></i></a></li>';
    }
    ...
    public function sn_email($url, $event)
    {
        ...
        return '<li class="mec-event-social-icon"><a class="email" href="mailto:?subject='.wp_specialchars_decode($event->data->title).'&body='.rawurlencode($url).'" title="'.esc_html__('Email', 'mec').'"><i class="mec-fa-envelope"></i></a></li>';
    }
    ...
    public function sn_vk($url, $event)
    {
        ...
        return '<li class="mec-event-social-icon"><a class="vk" href=" http://vk.com/share.php?url='.rawurlencode($url).'" title="'.esc_html__('VK', 'mec').'" target="_blank"><i class="mec-fa-vk"></i></a></li>';
    }
    ...
    public function sn_tumblr($url, $event)
    {
        $occurrence = (isset($_GET['occurrence']) ? sanitize_text_field($_GET['occurrence']) : '');
        if(trim($occurrence) != '') $url = $this->add_qs_var('occurrence', $occurrence, $url);
        return '<li class="mec-event-social-icon"><a class="tumblr" href="https://www.tumblr.com/widgets/share/tool?canonicalUrl='.rawurlencode($url).'&title'.wp_specialchars_decode($event->data->title).'&caption='.wp_specialchars_decode($event->data->title).'" title="'.esc_html__('Share on Tumblr', 'mec').'"><i class="mec-fa-tumblr"></i></a></li>';
    ...
    public function sn_pinterest($url, $event)
    {
        ...
        return '<li class="mec-event-social-icon"><a class="pinterest" href="http://pinterest.com/pin/create/button/?url='.rawurlencode($url).'" title="'.esc_html__('Share on Pinterest', 'mec').'"><i class="mec-fa-pinterest"></i></a></li>';
    ...
    public function sn_flipboard($url, $event)
    {
        ...
        return '<li class="mec-event-social-icon"><a class="flipboard" href="https://share.flipboard.com/bookmarklet/popout?v=2&title'.wp_specialchars_decode($event->data->title).'=&url='.rawurlencode($url).'" title="'.esc_html__('Share on Flipboard', 'mec').'">
    ...
    public function sn_pocket($url, $event)
    {
        ...
        return '<li class="mec-event-social-icon"><a class="pocket" href="https://getpocket.com/edit?url='.rawurlencode($url).'" title="'.esc_html__('Share on GetPocket', 'mec').'"><i class="mec-fa-get-pocket"></i></a></li>';
    ...
    public function sn_reddit($url, $event)
    {
        ...
        return '<li class="mec-event-social-icon"><a class="reddit" href="https://reddit.com/submit?url='.rawurlencode($url).'&title='.wp_specialchars_decode($event->data->title).'" title="'.esc_html__('Share on Reddit', 'mec').'"><i class="mec-fa-reddit"></i></a></li>';
    ...
    public function sn_telegram($url, $event)
    {
        ...
        return '<li class="mec-event-social-icon"><a class="telegram" href="https://telegram.me/share/url?url='.rawurlencode($url).'&text='.wp_specialchars_decode($event->data->title).'" title="'.esc_html__('Share on Telegram', 'mec').'">
    ...
    public function sn_whatsapp($url, $event)
    {
        ...
        return '<li class="mec-event-social-icon"><a class="whatsapp" href="whatsapp://send/?text='.rawurlencode($url).'" title="'.esc_html__('Share on WhatsApp', 'mec').'"><i class="mec-fa-whatsapp"></i></a></li>';
    ...
    public function do_endpoints()
    {
        ...
        {
            echo '<p class="mec-success">'.esc_html__('You returned from payment gateway successfully.', 'mec').'</p>';
        }
    ...
    public function booking_invoice()
    {
        ...
        if(isset($_GET['method']) and sanitize_text_field($_GET['method']) == 'mec-invoice')
        {
            ...
            {
                wp_die(__('Cannot find the invoice!', 'mec'), esc_html__('Invoice is invalid.', 'mec'));
                exit;
            }
            ...
            {
                wp_die(__('Your booking still is not confirmed. You can download it after confirmation!', 'mec'), esc_html__('Booking Not Confirmed.', 'mec'));
                exit;
            }

            if(!$event_id)
            {
                wp_die(__('Cannot find the booking!', 'mec'), esc_html__('Booking is invalid.', 'mec'));
                exit;
            }
            ...
            {
                wp_die(__("You don't have access to view this invoice!", 'mec'), esc_html__('Key is invalid.', 'mec'));
                exit;
            }
            ...
            if(isset($transaction['tickets']) and is_array($transaction['tickets']) and count($transaction['tickets']))
            {
                ...
                foreach($transaction['tickets'] as $attendee)
                {
                    ...
                    $pdf->Write(6, ((isset($event->tickets[$attendee['id']]) ? esc_html__($this->m('ticket', esc_html__('Ticket', 'mec'))).': '.esc_html($event->tickets[$attendee['id']]['name']) : '').' '.(isset($event->tickets[$attendee['id']]) ? $book->get_ticket_price_label($event->tickets[$attendee['id']], $booking_time, $event_id) : '')));
    ...
    public function cart_invoice()
    {
        ...
        foreach($cart as $transaction_id)
        {
            ...
            {
                $pdf->SetFont('DejaVuBold', '', 9);
                $pdf->Write(5, esc_html__('Date & Times', 'mec'));
                $pdf->Ln();
                $pdf->SetFont('DejaVu', '', 9);
            ...
            {
                $pdf->SetFont('DejaVuBold', '', 9);
                $pdf->Write(5, esc_html__('Date & Time', 'mec').': ');
                $pdf->SetFont('DejaVu', '', 9);
            ...
            if(isset($transaction['tickets']) and is_array($transaction['tickets']) and count($transaction['tickets']))
            {
                ...
                foreach($transaction['tickets'] as $attendee)
                {
                    ...
                    $pdf->Write(5, ((isset($event->tickets[$attendee['id']]) ? esc_html__($this->m('ticket', esc_html__('Ticket', 'mec'))).': '.esc_html($event->tickets[$attendee['id']]['name']) : '').' '.(isset($event->tickets[$attendee['id']]) ? esc_html($book->get_ticket_price_label($event->tickets[$attendee['id']], $booking_time, $event_id)) : '')));
    ...
    public function ical_email()
    {
        ...
        if(isset($_GET['method']) and sanitize_text_field($_GET['method']) == 'ical-email')
        {
            ...
            if($key != md5($book_id))
            {
                wp_die(__('Request is not valid.', 'mec'), esc_html__('iCal export stopped!', 'mec'), array('back_link'=>true));
                exit;
            }
    ...
    */
    ...
    public function get_flags($event, $date = NULL)
    {
        ...
        $output_tag = ' <span class="mec-event-title-soldout ' . sanitize_html_class($add_css_class) . '"><span class=soldout>%%title%%</span></span> ';
    ...
    {
        if($this->is_ongoing($event)) return '<dl><dd><span class="mec-holding-status mec-holding-status-ongoing">'.esc_html__('Ongoing...', 'mec').'</span></dd></dl>';
        elseif($this->is_expired($event)) return '<dl><dd><span class="mec-holding-status mec-holding-status-expired">'.esc_html__('Expired!', 'mec').'</span></dd></dl>';
