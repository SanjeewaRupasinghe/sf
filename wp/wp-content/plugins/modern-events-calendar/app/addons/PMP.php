...
class MEC_addon_PMP extends MEC_base
{
    ...
    public function booking_abort($abort, $event)
    {
        ...
        // Guest User
        if(!$user_id) return '<div class="warning-msg">'.sprintf(esc_html__('Please %s first to continue.', 'mec'), '<a href="'.wp_login_url($this->main->get_full_url()).'">'.esc_html__('login', 'mec').'</a>').'</div>';
