...
//        $this->enable_installments         = $this->get_option('enable_installments') == 'yes' ? true : false;
    ...
    function process_admin_options() {
        ...
        $applepaySettings['enabled']        = isset($settings['enable_installments']) ? $settings['enable_installments'] : "no";
        ...
        update_option( 'woocommerce_payfort_fort_applepay_settings', apply_filters( 'woocommerce_settings_api_sanitized_fields_applepay', $sadadSettings ) );
        return $result;
    }
    ...
    function init_form_fields()
    {
        $this->form_fields = array(
            ...
            'enable_applepay'        => array(
                'title'   => __('ApplePay', 'payfort_fort'),
                'type'    => 'checkbox',
                'label'   => __('Enable Apple Pay Payment Method', 'payfort_fort'),
                'default' => 'no'
            ),
        );
    }
    ...
    function process_payment($order_id)
    {
        ...
        if (!isset($_GET['response_code'])) {
            ...
            elseif($payment_method == PAYFORT_FORT_PAYMENT_METHOD_APPLEPAY) {
                $paymentMethod = PAYFORT_FORT_PAYMENT_METHOD_APPLEPAY;
            }
            ...
            if ($integrationType == PAYFORT_FORT_INTEGRATION_TYPE_REDIRECTION) {
                ...
                else if ($paymentMethod == PAYFORT_FORT_PAYMENT_METHOD_APPLEPAY) {
                    update_post_meta($order->id, '_payment_method_title', 'ApplePay');
                    update_post_meta($order->id, '_payment_method', PAYFORT_FORT_PAYMENT_METHOD_APPLEPAY);
                }
