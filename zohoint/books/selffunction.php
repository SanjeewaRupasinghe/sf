...
*/  
 // Global vars for Zoho API  
     $zoho_apis_com = "https://books.zoho.com/api/v3/";  
     $refresh_access_token_url = "https://accounts.zoho.com/oauth/v2/token";  
	 $organization_id = "717628836";
...
function getSalesContactId( $contact_name_contains, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_datasearch = $zoho_apis_com."contacts/?contact_name_contains=".$contact_name_contains."&organization_id=".$organization_id; 

	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url_datasearch);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
	curl_setopt($ch, CURLOPT_HTTPHEADER, $header_data);   
    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
	...
	curl_close($ch);
	$response_arr = json_decode($result, true);  
	$contact_id = $response_arr['contacts'][0]['contact_id'];
	return $contact_id;
}

function getContactPersId( $contact_id, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_data = $zoho_apis_com."contacts/".$contact_id."?organization_id=".$organization_id; 
	...
	return $contact_personsId;
}

function getInvoiceId( $orderno, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_data = $zoho_apis_com."invoices/?reference_number=".$orderno."&organization_id=".$organization_id; 
	...
	curl_close($ch);
	$response_arr = json_decode($result, true);  
	$invoice_id= $response_arr['invoices'][0]['invoice_id'];
	return $invoice_id;
}

function getSalesOrddByRef( $orderno, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_data = $zoho_apis_com."salesorders/?reference_number=".$orderno."&organization_id=".$organization_id; 
	...
	curl_close($ch);
	$response_arr = json_decode($result, true);  
	$salesorder_id = $response_arr['salesorders'][0]['salesorder_id'];
	return $salesorder_id;
}

function getCusIdByInvid( $invoice_id, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_data = $zoho_apis_com."invoices/".$invoice_id."?organization_id=".$organization_id; 
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url_data);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
...
function writeoffInv( $invoice_id, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_data = $zoho_apis_com."invoices/".$invoice_id."/writeoff?organization_id=".$organization_id; 
	...
	curl_close($ch);
	$response_arr = json_decode($result, true);  
	$customer_id = $response_arr['invoices'][0]['customer_id'];
	return $customer_id;
}

function markSorderOpen( $salesOrderID, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_data = $zoho_apis_com."salesorders/".$salesOrderID."/status/open?organization_id=".$organization_id; 
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url_data);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
...
function confirmSo( $salesorder_id, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_data = $zoho_apis_com."salesorders/".$salesorder_id."/approve?organization_id=".$organization_id;  
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url_data);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
	...
	$url_data2 = $zoho_apis_com."salesorders/".$salesorder_id."/status/confirmed?organization_id=".$organization_id; 

	$ch2 = curl_init();
	curl_setopt($ch2, CURLOPT_URL, $url_data2);
...
function markInvoiceApprove( $invoice_id, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_data = $zoho_apis_com."invoices/".$invoice_id."/approve?organization_id=".$organization_id; 
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url_data);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
	...
	$response_arr = json_decode($result, true);  
	$invsentstat = $response_arr['message'];
	return $invsentstat;
}



function markInvoiceSent( $invoice_id, $header_data ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
    $url_data = $zoho_apis_com."invoices/".$invoice_id."/status/sent?organization_id=".$organization_id; 
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url_data);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
...
function getitemId( $itemname, $header_data, $itemdata, $access_token ) 
     { 
    global $access_token_path, $zoho_apis_com, $organization_id;
	$url_data = $zoho_apis_com."items/?name_contains=".$itemname."&organization_id=".$organization_id; 	
	...
	curl_close($ch);
	$response_arr = json_decode($result, true);  
	$item_id = $response_arr['items'][0]['item_id'];
	...
	curl_setopt_array($curl, array(
	  CURLOPT_URL => "https://books.zoho.com/api/v3/items?organization_id=".$organization_id,
	  ...
	  CURLOPT_POSTFIELDS => array('JSONString' => $itemdata),
	...
	$item_id = $response_arr['item']['item_id'];;
